...
class HyperVolumePlotter:
	...
	def normalize_score(self):
		# unpack all scores to find the max and min
		all_scores = sorted(itertools.chain(*self.all_data['score']))
		ma = all_scores[-1]
		mi = all_scores[0]
		
		for i, iteration in enumerate(self.all_data['score']):
			iteration = np.subtract(iteration, mi)
			if ma - mi != 0:
				iteration = np.divide(iteration, ma-mi)
			# update array
			self.all_data['score'][i] = iteration
